...
image = imread('../data/shanghai.jpg');
sz = size(image);
...
% apply seam carving
tic
image_seamCarving_v = seamCarving(double(image), [sz(1), floor(sz(2)/2)]);
figure
imshow(uint8(image_seamCarving_v))
toc

tic
image_seamCarving_h = seamCarving(double(image), [floor(sz(1)/2), sz(2)]);
figure
imshow(uint8(image_seamCarving_h))
toc
